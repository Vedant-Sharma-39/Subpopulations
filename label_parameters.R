...
# Make a csv file of total population time series and assigned cluster

total_pop_cluster_results <- lapply(
  list.files("Data/TotalPopClusteringResults", full.names = TRUE), 
  function (file_path) {
    clust  <- get(load(file_path))
    return(clust@cluster)
  })

# For each clustering save a csv file with total population and assigned cluster

for (i in seq_along(total_pop_cluster_results)) {
  cluster_result <- total_pop_cluster_results[[i]]
  cluster_result_df <- data.frame(param_sets, cluster = cluster_result)
  write.csv(cluster_result_df, paste0("Data/TotalPopClusteringResults/cluster_", i+1, ".csv"))
}